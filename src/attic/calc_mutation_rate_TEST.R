...
inp <- unique(metadata$patient) %>%
  purrr::map(function(x) get_cum_num_mut(x, dat)) %>%
  bind_rows()

dists <- inp %>%
  group_by(patient) %>%
  filter(num_out == max(num_out)) %>%
  ungroup() %>%
  mutate(
    IQR = IQR(num_out),
    O_upper = quantile(num_out, probs = 0.75, na.rm = FALSE) + 3 * IQR,
    O_lower = quantile(num_out, probs = 0.25, na.rm = FALSE) - 3 * IQR
  ) %>%
  filter(O_lower <= num_out & num_out <= O_upper) %>%
  pull(num_out)
