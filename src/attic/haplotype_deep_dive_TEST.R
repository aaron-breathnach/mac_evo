...
viz_hf_x_maf <- function(pid, metadata, mat_hap, mat_snp, hap_x_snp) {
  ...
  out_a <- purrr::map(1:length(out), function(x) out[[x]][1]) %>%
    bind_rows()
  
  out_b <- purrr::map(1:length(out), function(x) out[[x]][2]) %>%
    bind_rows()
  
  list(out_a, out_b)
  
  # plotlist <- list()
  # for (i in 1:8) plotlist[i] <- plot.new()
  # 
  # for (i in 1:length(plotlist)) {
  #   panel <- panels[i]
  #   plotlist[panel] <- plot_list[i]
  # }
  # 
...
dat <- purrr::map(pids, function(x) viz_hf_x_maf(x, metadata, mat_hap, mat_snp, hap_x_snp))

p_inp_a <- bind_rows(purrr::map(1:16, function(x) dat[[x]][[1]]))
p_inp_b <- bind_rows(purrr::map(1:16, function(x) dat[[x]][[2]]))

p <- ggplot(p_inp_a, aes(x = time_from_diagnosis, y = frequency)) +
  facet_grid(haplotype ~ patient, scales = "free_x", space = "free_x") +
  geom_line(aes(group = id),
            show.legend = FALSE,
            linewidth = 0.1,
            colour = "steelblue",
...
ggsave("output/hf_x_af.png", p, height = 12.5, width = 20)
