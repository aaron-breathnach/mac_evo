...
make_figure_3a <- function(x, metadata) {
  
  p_inp <- inner_join(x, metadata, by = "isolate") %>%
...
make_figure_3b <- function(x, metadata) {
  
  df <- run_check_haplo_status(x, metadata) %>%
    group_by(patient) %>%
...
make_figure_3 <- function(
    DAT = "data/haplotype_deconstructor.80_percent.RDS",
  ...
  p_a <- make_figure_3a(dat[[2]], metadata)
  p_b <- make_figure_3b(dat[[2]], metadata)
