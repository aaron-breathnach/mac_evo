...
make_figure_3b <- function(metadata, snp_dists, pal) {
  ...
  igraph::V(g)$x <- layout[, 1][[1]]
  igraph::V(g)$y <- layout[, 2][[1]]
...
make_figure_3c <- function(tree, metadata, fastbaps, snp_dists, pal, save = FALSE) {
...
make_figure_3 <- function() {
  ...
  p_b <- make_figure_3b(metadata, snp_dists, pal)
  p_c <- make_figure_3c(tree, metadata, fastbaps, snp_dists, pal)
  ...
  figure_3 <- cowplot::plot_grid(part_1, part_2, nrow = 2, rel_heights = c(1, 1.75), scale = 0.95)
  
  ggsave("plots/figure_3.png", figure_3, width = 16.25, height = 10, bg = "white")
