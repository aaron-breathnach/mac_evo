...
make_figure_4a <- function(relab, metadata) {
...
make_figure_4b <- function(relab, metadata) {
  ...
  ggplot(p_inp, aes(x = patient, y = n, label = n, group = status)) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          axis.title = element_text(face = "bold"),
          legend.title = element_text(face = "bold")) +
    scale_fill_manual(values = wesanderson::wes_palette("Chevalier1", 3)) +
...
make_figure_4 <- function() {
  ...
  p_a <- make_figure_4a(relab, metadata)
  p_b <- make_figure_4b(relab, metadata)
  
  p <- cowplot::plot_grid(p_a, p_b, nrow = 2, labels = "AUTO", scale = 0.95)
  
  ggsave("plots/figure_4.png", p, width = 12.5, height = 8.75, bg = "white")
